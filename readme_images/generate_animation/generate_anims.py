...
if __name__ == '__main__':
    ...
    ds = data['ds']
    photon_energies = np.linspace(0.1, 0.75, num=651)
    thetas = np.linspace(0,90, num=181)
    if RANK == 0:
        print('hlloooo')
        RTEs = np.zeros((ds.shape[0], photon_energies.shape[0], thetas.shape[0]))
        RTMs = np.zeros((ds.shape[0], photon_energies.shape[0], thetas.shape[0]))
        foms = np.zeros((ds.shape[0]))
    ...
    if RANK==0:
        ...
        rects = []
        for k in range(ds.shape[1]-2):
            if k%2 == 0:
                rect = Rectangle([0,0],1.0,1.0,ec=[0,0,0],fc=[0.153,0.227,0.373],lw=0.5)
                #rect = Rectangle([0,0],1.0,1.0,ec='k',fc='k',lw=1)
            else:
                rect = Rectangle([0,0],1.0,1.0,ec=[0,0,0],fc=[0.871,0.922,0.98],lw=0.5)
                #rect = Rectangle([0,0],1.0,1.0,ec='k',fc='b',lw=1)
            rects.append(rect)

        for k in range(len(rects)):
            ax1.add_patch(rects[k])

        init_cmap = np.zeros((photon_energies.shape[0], thetas.shape[0]))
        pe0 = photon_energies[0]
        pe1 = photon_energies[-1]
        ia0 = thetas[0]
        ia1 = thetas[-1]
        im3 = ax3.imshow(init_cmap, extent=[ia0,90.0,pe0,pe1], aspect='auto', origin='lower', cmap='gnuplot2', vmin=0.0, vmax=100.0)
        im4 = ax4.imshow(init_cmap, extent=[ia0,90.0,pe0,pe1], aspect='auto', origin='lower', cmap='gnuplot2', vmin=0.0, vmax=100.0)
        c3 = f.colorbar(im3, ax=ax3)
        c4 = f.colorbar(im4, ax=ax4)
        ax3.set_ylabel('Photon Energy (eV)')
        ax4.set_ylabel('Photon Energy (eV)')
        ax3.set_xlabel('Incident Angle (degrees)')
        ax4.set_xlabel('Incident Angle (degrees)')
        c3.ax.set_ylabel('Reflectivity (\%)')
        c4.ax.set_ylabel('Reflectivity (\%)')

        dsmax = np.max(np.sum(ds,axis=1))*1e6


        def animate(i):
           ax2.clear()
           ax2.plot(-100*foms[:i+1], 'k')
           ax2.set_ylim([65.0,95.0])
           #ax2.set_xlim([0.0, ds.shape[0]])
           ...
           im3.set_data(100*np.squeeze(RTEs[i, ...]))
           im4.set_data(100*np.squeeze(RTMs[i, ...]))
        
           #dd = np.flipud(np.squeeze(ds[i,1:-1]))*1e6
           dd = np.squeeze(ds[i,1:-1])*1e6
           #dsum = 0
           #for k in range(len(rects)-1,-1,-1):
           #    rects[k].set_xy([0,dsum])
           #    rects[k].set_width(1.0)
           #    rects[k].set_height(dd[k])
           #    dsum += dd[k]
           dsum = dsmax
           for k in range(len(rects)):
               rects[k].set_xy([0.1,dsum-dd[k]-0.1])
               rects[k].set_width(0.8)
               rects[k].set_height(dd[k])
               dsum -= dd[k]
           ax1.set_ylim([0.0, dsmax])

        #ani = FuncAnimation(f, animate, frames=ds.shape[0], interval=40, repeat=False, save_count=ds.shape[0])
        ani = FuncAnimation(f, animate, frames=150, interval=60, repeat=False, save_count=150)
