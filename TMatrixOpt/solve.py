...
class SimpleTMatrix:
    ...
    def incident_angle(self, val):
        self._incident_angle = val
    ...
    def solve(self, photon_energies, thetas):
        ...
        #    dM_dd_TM[count, ...] = output[3]
        #    count += 1
        output = self.__solve_BETA(simulations, len_sim)
        M_TE = output[0]
        M_TM = output[1]
        dM_dd_TE = output[2]
        dM_dd_TM = output[3]
    ...
    def __solve_BETA(self, simulations, len_sim):
        assert self.__build

        d = self.__d
        idx = np.array(self.__idx, dtype=np.int32)
        nn = np.empty((len(self.__n), len_sim), dtype=np.complex128)
        n = np.empty(len(self.__n)*len_sim, dtype=np.complex128)
        photon_energy = np.array([simulations[i][0] for i in range(len_sim)])
        theta = np.array([simulations[i][1] for i in range(len_sim)])
        ...
        for i in range(len(self.__n)):
            if type(self.__n[i]) is Material:
                nn[i,:] = self.__n[i].get_index(photon_energy)
            else:
                nn[i,:] = self.__n[i,:]
        ...
        M_TE = np.empty(4*len_sim, dtype=np.complex128)
        M_TM = np.empty(4*len_sim, dtype=np.complex128)
        dM_dd_TE = np.empty(4*len_idx*len_sim, dtype=np.complex128)
        dM_dd_TM = np.empty(4*len_idx*len_sim, dtype=np.complex128)

        lib.solve_BETA(photon_energy,
                       theta,
                       len_d,
                       len_idx,
                       len_sim,
