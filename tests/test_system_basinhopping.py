...
if __name__ == '__main__':
    ...
    layer2 = geometry.LinearChirp('linearchirp', 0.19, 0.8, 8, 1.6, 3.5, 1.6, 3.5, True)
    ...
    photon_energies = np.linspace(0.1, 0.74, num=2000)
    ...
    start = time.time()
    #stack.check_gradient(params, step=5e-9)
    end = time.time()
    print('time')
    print(end-start)
    ...
    additional_options = {'stepsize':0.5e-13, 'beta':0.9}
    opt = optimizer.basin_hopping(stack, params, callback_func=callback, opt_method=opt_method, Nmax=75, tol=1e-10, bounds=bounds, scipy_verbose=True, additional_options=additional_options)
    ...
    if RANK == 0:
        ...
        scipy.io.savemat('results_Np8_bh.mat', to_save)
        stack.print_info()
        stack.plot_fom()
