...
if __name__ == '__main__':
    ...
    layer2 = geometry.LinearChirp('linearchirp', 0.19, 0.8, 4, 1.6, 3.5, 1.6, 3.5, True)
    ...
    thetas = np.linspace(0,90, num=900)
    ...
    additional_options = {'stepsize':5e-14, 'beta':0.9}
    opt = optimizer.Optimizer(stack, params, callback_func=callback, opt_method=opt_method, Nmax=200, tol=1e-10, bounds=bounds, scipy_verbose=True, additional_options=additional_options)
    ...
    if RANK == 0:
        ...
        scipy.io.savemat('results_Np4.mat', to_save)
        stack.print_info()
        stack.plot_fom()
